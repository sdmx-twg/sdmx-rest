...
--- | --- | --- | ---
detail | *String* | This attribute specifies the desired amount of information to be returned. For example, it is possible to instruct the web service to return only basic information about the maintainable artefact (i.e.: id, agency id, version and name). Most notably, items of item schemes will not be returned (for example, it will not return the codes in a code list query). Possible values are: (1) `allstubs`: all artefacts should be returned as stubs, containing only identification information, as well as the artefacts' name. (2) `referencestubs`: referenced artefacts should be returned as stubs, containing only identification information, as well as the artefacts' name. (3) `referencepartial`: referenced item schemes should only include items used by the artefact to be returned. For example, a concept scheme would only contain the concepts used in a DSD, and its isPartial flag would be set to true. Likewise, if a dataflow has been constrained, then the codelists referenced by the DSD referenced by the dataflow should only contain the codes allowed by the content constraint. (4) `allcompletestubs`: all artefacts should be returned as complete stubs, containing identification information, the artefacts' name, description, annotations and isFinal information. (5) `referencecompletestubs`: referenced artefacts should be returned as complete stubs, containing identification information, the artefacts' name, description, annotations and isFinal information. (6) `raw`: returned codelists should include the ExtendedBy property, thereby allowing clients to see the inherited codelists, if any. (7) `partialraw`: referenced item schemes should only include items used by the artefact to be returned, as for the value 'referencepartial'. In addition, returned codelists should include the ExtendedBy property, thereby allowing clients to see the inherited codelists, if any, but only those used by the artefact are to be returned. (8) `full`: all available information for all artefacts should be returned. This must return a resolved version of codelists, i.e. codelists without the `ExtendedBy` property and with all inherited codes. | **full**
